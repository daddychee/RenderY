...
### 2026-09-05 — PROCESSING V2: chia khối 2 cột + lịch sử theo ngày + chi phí/tập + mở report từ web

**Cái gì đổi:** ① `web/static/index.html` tab Processing viết lại theo mockup user duyệt 05/09 (`static/mockup_processing.html`, mở `/mockup-processing`): 4 chips (máy đang dựng X/Y · tập xếp hàng · việc chờ tay · $ AI hôm nay) + lưới 2 cột — trái: thẻ ĐANG CHẠY có thanh tiến độ % + «còn ~X phút» (dùng `progress`/`eta` backend ĐÃ tính sẵn mà UI cũ bỏ phí), phải: Cần bạn xử lý + Hàng chờ dạng danh sách dọc — + LỊCH SỬ gộp `<details>` theo ngày (chỉ Hôm nay mở sẵn; nhớ nhóm/dòng đang mở qua vòng poll 7s), ấn dòng mở chi tiết: thời gian dựng, chi phí, link report từng chương (tab mới), đường NAS + nút Copy (lazy fetch `/api/ketqua` lần đầu mở), log. Khối «Đã giao gần đây» + modal `xemKetQua` bỏ — lịch sử thay trọn. ② `web/server.py::api_jobs` thêm `chi_phi`/job (đọc `aigen_duyet.json` sẵn có: ảnh $0.03/phương án không `_ref` + video $0.05/motif có ảnh chốt khi `da_gen_video` — tính Ở SERVER để job xong rồi lịch sử vẫn còn số, UI cũ chỉ tính được job sống) + `report_ok`/chương; endpoint mới `GET /api/report/{project_id}` serve report.html từ `report_path` trong project.json; route `/mockup-processing`.

**Vì sao:** user 05/09: trang Processing khó nhìn, mọi thứ đổ một cột dài; cần 6 điều: ai chạy · đến đâu · hàng chờ · lịch sử kèm report ấn mở luôn · gọn theo ngày có dropdown · chi phí mỗi tập. Duyệt qua 2 vòng mockup (v1 một cột → user chê dài → v2 chia 2 cột, chốt).

**Verify:** `py_compile` OK; smoke in-process bằng TestClient KHÔNG context manager (lifespan không chạy → KHÔNG sinh worker mới — lúc sửa đang có 2 job thật id 14/15 chạy): `/api/jobs` 200 trả chi_phi + report_ok + stats; `/api/report/<pid có report>` 200 (228KB html), `<pid thiếu>` 404 đúng chữ; `/mockup-processing` 200. JS: ngoặc {}/()/[]cân, mọi handler gọi từ onclick đều tồn tại. index.html là file tĩnh → F5 ăn ngay; **server.py cần restart web lúc máy rảnh** thì chi phí lịch sử + link report mới sống — trước đó UI tự fallback («—» / «chưa có report»), không vỡ.

**Vùng ảnh hưởng đã rà:** tab Generation / Framing Insight / New Sequence không đụng (`tienPhien`/`PHIEN`/`veSelectPhien`/`moPhien` giữ nguyên chữ ký); `.tag.loi` đổi từ viền sang nền soft (dùng chung ở chips kiểm tập — chỉ thẩm mỹ); `.viec` viết lại nhưng chỉ Processing dùng; `api_jobs` chỉ THÊM field — client cũ không đọc thì bỏ qua; uvicorn chạy không reload nên sửa server.py không tự restart giữa chừng.

**Ghi chú / còn ngỏ:** lịch sử theo giới hạn 50 job sẵn có của `list_jobs` — cần xem sâu hơn thì thêm `?limit`/nút «tải thêm» sau; «AI đã tiêu hôm nay» tính từ 50 job đó (đủ vì 1 ngày không quá 50 job).

**Bổ sung cùng ngày — AI LÀM khi vào thẳng:** thẻ Đang chạy hiện «—» vì `nguoi` chỉ lấy từ SSO CRM (X-Remote-User); vào thẳng bằng token thì server không biết ai nộp. Sửa: header UI có «Khai tên bạn» (prompt, lưu localStorage `rendery_nguoi` theo máy, có «đổi tên»), `nopTap` gửi kèm `nguoi`; `JobRequest.nguoi` mới + `api_add_job` dùng `current_user(request) or req.nguoi[:40]` — qua CRM header vẫn thắng, không giả mạo được. Client cũ không gửi field → mặc định rỗng như cũ; server cũ (chưa restart) nhận field thừa → pydantic bỏ qua, vô hại. Job nộp TRƯỚC restart vẫn lưu rỗng.

**Bổ sung cùng ngày — 3 việc user báo chiều 05/09:** ① Job 15 (LI096_Hai) fail: root cause GLM HẾT TIỀN mã 1113 (z.ai) giữa direct chia beat 4/12 — không phải bug code; kèm cảnh báo align 91%<95% từ khớp (nghi writer sửa script sau khi gen voice) là WARNING không gây fail. Hành động: nạp tiền z.ai → nộp lại; stage đã xong được giữ. ② Framing khó đọc: hồ sơ fern là mô tả ĐỜI ĐẦU (3-4 câu/trường dính liền — sinh trước luật VIẾT NGẮN trong `kenh/mo_ta.py`); UI thêm `ngatCau` (mỗi câu một dòng, áp cho la_gi + thẻ 4 mặt + giao việc + ky_thuat) đọc được ngay cả hồ sơ cũ; fern nên bấm «Viết lại mô tả» SAU khi nạp tiền GLM (1 call, không tải video). ③ Frame minh hoạ: ĐÃ có từ sáng 05/09 (`do_kenh::_luu_frames` → `<kenh>/frames/f*.jpg`, tokdiscovery đủ 6 tấm 14:03; route `/api/kenh/{ten}/frame/{so}` + `so_frame` trong api_kenh_list + dải `fi-frames` UI) — chưa hiện vì SERVER PROCESS CŨ chưa restart; fern đo trước tính năng nên muốn frame phải «Đo lại». ④ tenTap bỏ đuôi «RenderY» (quy ước <tập>/RenderY) — thẻ job hết hiện tên tập là «RenderY».

**Bổ sung cùng ngày — nối Serper vào két V3 (route entity):** két CRM vốn ĐÃ hỗ trợ loại "serp" nhà Serper.dev (tab Add API), chỉ thiếu 2 mắt xích: ① hợp đồng `nen/rules/apps.json` (OUTLIERY-V3, có backup .bak) — rendery thêm việc `tim_tu_lieu` («Tìm tư liệu thật (entity)», loai=serp); hợp đồng đọc sống theo mtime nên KHÔNG cần restart CRM. ② `web/ket_v3.py::nap_env` map việc `tim_tu_lieu` → `SERPER_API_KEY` (chỉ nhà "serper" — serpapi/searchapi chưa có client, bỏ qua); `sourcer/entity.py` đọc biến này, load_dotenv không đè env nên két thắng .env. Test: +2 (`test_serper_dat_dung_bien`, `test_nha_serp_khac_thi_bo_qua`), test_ket_v3 24/24. Hiệu lực cần restart web RenderY (nap_env chạy trong process server — gộp lần restart đang chờ LI100). User cùng lúc thêm key Pexels+Pixabay thứ 2 vào két (xoay vòng, nhân hạn mức search ×2 — ăn ngay không cần code).

**Kích hoạt cùng ngày 15:2x:** restart app rendery theo runbook CRM (kill PID cổng 9118 → `tools\scripts\start-all.ps1`, chỉ bật lại app chết — các app khác nguyên). `requeue_orphans` hoạt động đúng thiết kế: job 14 (đang report) + job 16 (đang direct) tự về hàng đợi và được nhận lại ngay, stage DONE tự bỏ qua. Verify sau restart: /health ok · /api/jobs có chi_phi · so_frame tokdiscovery=6 + frame serve 200 · /api/report 200 (228KB) · /mockup-processing 200. Từ giờ mọi thứ ăn trọn: chi phí lịch sử, mở report, frame Framing, ngắt câu mô tả, tên người làm, key Serper qua két.

**Sự cố cùng ngày 15:3x — TOKEN NỘI BỘ làm job mất khoá GLM sau restart:** CRM 05/09 thêm lớp token nội bộ (nen/common/token_noi_bo.py, GĐ6 docs/bao-mat-internet.md): route phát khoá `/api/cau-hinh/api-khoa/*` đòi header `X-Noi-Bo` khớp `OUTLIERY_TOKEN_NOI_BO` — start-all.ps1 SINH MỚI mỗi lần chạy, cả cụm kế thừa chung từ tiến trình cha (thiết kế: restart CẢ CỤM cùng lúc). 9 caller trong CRM đã migrate; **RenderY (repo ngoài) bị sót** + gateway restart lẻ 15:04 và rendery restart lẻ 15:2x → mỗi process một token → két từ chối ("thieu token noi bo") → ket_v3 fail-open về .env (không có khoá) → job 16 chạy lại chết "Thiếu khoá GLM". Sửa: ① `ket_v3.doc_ket` gửi header `X-Noi-Bo` từ env (không có biến = không gửi, máy dev vẫn chạy — đúng khuôn token_noi_bo.header()); test +1, test_ket_v3 25/25. ② Restart CẢ CỤM đúng thiết kế: stop-all.ps1 → start-all.ps1 (16 dịch vụ, một token chung). ③ Nộp lại LI096_Hai (job 17, opts y job 16). **LUẬT VẬN HÀNH rút ra: từ nay KHÔNG restart lẻ gateway hay app nào có gọi két — muốn restart là stop-all → start-all cả cụm** (hoặc CRM đổi sang token bền trong .env thì mới hết ràng buộc này — việc của phía CRM, GĐ6 ghi "đặt token trong start-all rồi siết").

**Bổ sung chiều 05/09 — Framing: kho video bền + mốc thời gian đồ thị (3 việc user báo):** ① fern «đo lại vẫn không có frame/nhịp»: hồ sơ fern đo 11:53 — TRƯỚC khi _luu_frames + nhip_curve ra đời giữa trưa (tokdiscovery 13:45 có đủ) — không phải bug, chỉ cần đo lại bằng code hiện tại. ② «Đo lại bắt tải lại video»: bỏ thiết kế thư mục tạm + rmtree trong `kenh/do_kenh.py` — video 360p giữ BỀN ở `<kenh>/videos/`; do_kenh thấy kho có video là phân tích thẳng (ghi log «dùng lại N video trong kho»), kho trống mới gọi yt-dlp (tải thẳng vào kho). Đo lại = tái phân tích tức thì, thêm thước mới sau này khỏi đụng YouTube (né chặn IP). Giá đĩa ~300-500MB/kênh (C: còn 72GB), xoá kênh là kho đi theo. Test `test_do_kenh_do_that_va_cache` ĐỔI LUẬT: do_lai không được tải lại khi kho có video. `api_kenh_do_lai` hết hardcode so_video=5 — giữ quy mô cũ (len(nguon), kẹp 5..8). ③ «Nhịp theo mốc thời gian làm input dựng»: HoSoKenh thêm `dai_trung_vi_s` (median tong_s video hội tụ); UI `veChart` nhận dai_s — trục x ghi 5 mốc mm:ss thật (0:00 → hết bài) + chú thích «mỗi mốc ≈ Xs» cho cả đồ thị nhịp lẫn nhạc; hồ sơ cũ thiếu trường thì rơi về nhãn đầu/cuối như cũ (doc() lọc key, không vỡ). Verify: pytest kenh+ket_v3 44/44; đã tải bộ video fern vào kho (lần cuối phải tải — kho cũ bị thiết kế cũ xoá) + kích đo lại qua server (giữ GLM key cho loại cảnh + mô tả). LƯU Ý: server đang chạy nạp `do_kenh` LƯỜI nên code mới ăn ngay lượt đo đầu tiên; riêng `api_kenh_do_lai` (so_video) trong server.py cần restart cụm kế tiếp mới ăn — không ảnh hưởng vì kho có video thì tham số này không dùng tới.

**Chốt 16:46 — fern đo lại thành công từ kho video:** sau 2 lần đứt vì cụm bị restart song song (16:05 còn dính 1 instance data-root lạ trả 404), lượt đo 16:26 chạy trọn trên server hiện hành: phân tích thẳng 8 video trong kho (không tải lại), hồ sơ mới có nhip_curve 24 bucket + dai_trung_vi_s 1723s (28:43) + 6 frame + mô tả gọn, giữ nguyên 8 video hội tụ. Toàn bộ 3 yêu cầu Framing chiều 05/09 đã lên production.
